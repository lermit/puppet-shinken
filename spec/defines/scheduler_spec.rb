...
describe 'shinken::scheduler', :type => :define do
  ...
  describe 'Test standard usage creation' do
    ...
    it { should contain_file('shinken-config-schedulers/master.cfg').with_content(/skip_initial_broks 0/) }
  end
  ...
  let(:params) { {
    ...
    it { should contain_file('shinken-config-schedulers/master.cfg').with_content(/modules module1,module42/) }
  end
  ...
  let(:params) { {
    ...
    it { should contain_file('shinken-config-schedulers/master.cfg').with_content(/satellitemap sat1,sat42/) }
  end
end
